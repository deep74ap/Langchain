...
class Feedback(BaseModel):
    ...
    #Field(descreption) helps to define what should be the output llm need to give in this section
    summary: str = Field(description="A brief summary for the provided feedback")

    #Literal helps to categorize the sentiments based on  the feeddback
    sentiment: Literal["pos" , "neg" , "neutral"] = Field(description="Return the sentiment and overall tone of review")

    #Optional helps LLM to decide whether give output for this section or not based on data
    
    Rating :   Optional[int] = Field(default=None, description="Rating of the car based on feedback within range of 1 to 5" , gt = 0 ,lt=6)
...
#Type change for res that is pydantic object

st_dict = dict(res)
st_json = res.model_dump_json()
print(st_json)